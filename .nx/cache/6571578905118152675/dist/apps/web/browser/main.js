...
var appRoutes = [
  ...
  {
    path: "login",
    loadComponent: () => import("./chunk-5ANE46XS.js").then((m) => m.LoginComponent)
  },
  {
    path: "register",
    loadComponent: () => import("./chunk-5ANE46XS.js").then((m) => m.RegisterComponent)
  },
  {
    path: "forgot-password",
    loadComponent: () => import("./chunk-5ANE46XS.js").then((m) => m.ForgotPasswordComponent)
  },
  // ─── Root ────────────────────────────────────────────────────────────────
  {
    path: "",
    loadComponent: () => import("./chunk-452LVTNZ.js").then((m) => m.HomeComponent)
  },
...
var SAAS_CONFIG = {
  ...
  // ─── Auth-Features ──────────────────────────────────────────────────────────
  // Steuert das Verhalten des firebase-auth Moduls
  auth: {
    publicSignup: true,
    // false → /register deaktiviert (Invite-Only oder Admin-Only)
    googleLogin: true,
    // false → Google-Button ausgeblendet
    emailLogin: true,
    // false → nur Social Login
    inviteOnly: false,
    // true → Signup nur via /invite/:token (überschreibt publicSignup)
    passwordReset: true,
    // false → "Passwort vergessen" Link ausgeblendet
    emailVerification: true
    // false → keine Verifikationsmail nach Registrierung
  },
  // ─── App-Features ───────────────────────────────────────────────────────────
  // Steuert welche App-Bereiche aktiv sind
  features: {
    onboarding: true,
    // false → direkt ins Dashboard nach Registrierung (kein Org-Wizard)
    multiOrganization: true,
    // false → kein Org-Switcher, kein Org-Management
    billing: true,
    // false → /pricing + /billing deaktiviert, keine Plan-Guards
    adminPanel: true,
    // false → /admin/** komplett deaktiviert
    trialDays: 14,
    // 0 → kein Trial-Zeitraum
    invitations: true,
    // false → Mitglieder können nicht eingeladen werden
    notifications: true
    // false → Notification Bell ausgeblendet
  },
  // ─── Legal ──────────────────────────────────────────────────────────────────
  legal: {
...
var appConfig = {
  providers: [
    ...
    provideFirebaseAuth({
      ...
      loginRoute: "/login",
      // Auth-Feature-Flags kommen direkt aus der zentralen SAAS_CONFIG
      features: SAAS_CONFIG.auth
    })
  ]
};
